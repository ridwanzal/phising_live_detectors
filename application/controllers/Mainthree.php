...
class Mainthree extends CI_Controller {
	...
	public function index()
	{
		...
		$query2 = "SELECT * FROM ph_smart_features2 ORDER BY id DESC";
	...
	public function scan(){
		...
		if($this->db->affected_rows() > 0){
            ...
            // rules
            $xrule_a = $this->rule_a($url);
            $xrule_b = $this->rule_b($url);
            $xrule_c = $this->rule_c($url);
            $xrule_d = $this->rule_d($url);
            $xresult = '';
			...
			$b_f7 = $this->read_html_filesize($file_path);
			$b_f8 = $this->read_html_favicon($url);
            ...
            // hitung jumlah rule yang true jika salah satu bernilai true maka web ter
            // indikasi phishing
            $arrayof_true2 = array($xrule_a, $xrule_b, $xrule_c, $xrule_d, $xrule_e);
            $counts2 = array_count_values($arrayof_true2);
            $count_true2 =  $counts2[1];
			...
			$this->db->insert('ph_smart_features2', $feature_data2);
    ...
    /*
        Change rules
        Rule A ; Mengecek jika terdapat fitur login palsu
        Rule B : Mengecek double top domain
        Rule C : Mengecek jika ada fitur banyak dot dan dash
        Rule D : Protocol http dan top target
        Rule E : Mengecek jika ada 3 atau lebih fitur terdeteksi.
    */

    public function rule_a($uri){
        $a = $this->cek_fakelogin($uri);
        if($a){
            return 1;
        }else{
            return 0;
    ...
    public function rule_d($uri){
        $a = $this->cek_http($uri);
        $b = $this->read_toptarget($uri);
        if($a && $b){
            return 1;
        }else{
            return 0;
